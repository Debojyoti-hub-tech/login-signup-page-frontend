...
const AnimatedNavLink = ({ href, children }: { href: string; children: React.ReactNode }) => {
  ...
  return (
    <a
      href={href}
      className={`group relative inline-block overflow-hidden h-5 leading-5 whitespace-nowrap ${textSizeClass}`}
      aria-label={typeof children === 'string' ? children : undefined}
    >
      <div
        className="flex flex-col transform transition-transform duration-300 ease-out group-hover:-translate-y-1/2"
...
function MiniNavbar({ onLoginClick, onSignupClick }: { onLoginClick?: () => void; onSignupClick?: () => void }) {
  ...
  const loginButtonElement = (
    <button onClick={() => onLoginClick && onLoginClick()} className="px-4 py-2 sm:px-3 text-xs sm:text-sm border border-[#333] bg-[rgba(31,31,31,0.62)] text-gray-300 rounded-full hover:border-white/50 hover:text-white transition-colors duration-200 w-full sm:w-auto">
      LogIn
    </button>
  );
  ...
  const signupButtonElement = (
    <div className="relative group w-full sm:w-auto">
       ...
       <button onClick={() => onSignupClick && onSignupClick()} className="relative z-10 px-4 py-2 sm:px-3 text-xs sm:text-sm font-semibold text-black bg-gradient-to-br from-gray-100 to-gray-300 rounded-full hover:from-gray-200 hover:to-gray-400 transition-all duration-200 w-full sm:w-auto">
         Signup
       </button>
    </div>
  );
  ...
  const handleEmailSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      if (!firstName.trim() || !lastName.trim() || !email.trim() || !password) {
        if (!firstName.trim()) { (document.getElementById('first-name') as HTMLInputElement | null)?.focus(); return; }
        if (!lastName.trim()) { (document.getElementById('last-name') as HTMLInputElement | null)?.focus(); return; }
  ...
  return (
    <div className={cn("flex w-[100%] flex-col min-h-screen bg-black relative", className)}>
      ...
      <div className="relative z-10 flex flex-col flex-1">
        <MiniNavbar onLoginClick={() => { setMode('login'); setStep('email'); }} onSignupClick={() => { setMode('signup'); setStep('email'); }} />
                ...
                {step === "email" ? (
                  ...
                  >
                    <div className="space-y-1">
                      <h1 className="text-[2.5rem] font-bold leading-[1.1] tracking-tight text-white">{mode === 'login' ? 'Welcome Developer' : 'Create an Account'}</h1>
                      <p className="text-[1.8rem] text-white/70 font-light">{mode === 'login' ? 'Your Log In component' : 'Your signup component'}</p>
                    </div>
                        ...
                        <div className="space-y-3">
                          {mode === 'signup' && (
                            <div className="flex gap-3 items-stretch">
                              <input
                                ...
                                className="flex-1 -ml-2 self-stretch box-border h-12 bg-transparent text-white/60 placeholder:text-white/50 border border-white/20 rounded-full px-4 focus:outline-none focus:border-white/20 text-center transition-colors appearance-none"
                              />
                              <input
                                id="last-name"
                                type="text"
                          ...
                          <div className="relative">
                            <input
                              ...
                              className={cn(
                                "w-full self-stretch box-border h-12 bg-transparent text-white/60 placeholder:text-white/50 border border-white/20 rounded-full px-4 focus:outline-none focus:border-white/20 text-center transition-colors appearance-none",
                                mode === 'login' ? 'pr-12' : ''
                              )}
                              required
                            />
                            ...
                            {mode === 'login' && (
                              <button type="submit" aria-label="LogIn" className="absolute right-2 top-1/2 transform -translate-y-1/2 w-10 h-10 rounded-full bg-white/6 border border-white/10 flex items-center justify-center text-white transition-colors hover:bg-white/10">
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-[1.25rem] text-white/50 font-light">Please enter the 6-digit code we sent to {email}</p>
                    </div>
